...
def load_menu(_client, sheet_url):
    try:
        ...
        for row in rows[1:]:
            ...
            pm = clean_p(row[idx_m]) if idx_m != -1 and idx_m < len(row) else None
            pl = clean_p(row[idx_l]) if idx_l != -1 and idx_l < len(row) else None
            pp = clean_p(row[idx_p]) if idx_p != -1 and idx_p < len(row) else None
...
def load_balances(_client, sheet_url):
    try:
        ...
        idx_name = -1
        for k in ["姓名", "Name", "員工"]:
            if k in headers: idx_name = headers.index(k); break
        
        idx_bal = -1
        for k in ["存款餘額", "餘額", "存款"]:
            if k in headers: idx_bal = headers.index(k); break
...
def upload_to_drive(pdf_bytes, filename, s_info):
    try:
        ...
        if not folder_id:
            st.error("❌ 上傳失敗：找不到 `drive_folder_id`。請檢查 Secrets 設定。")
            return None
        ...
        if "storageQuotaExceeded" in error_msg:
            st.error(f"❌ 權限錯誤：機器人無法寫入。請確認資料夾 ID `{folder_id}` 已共用給 `{s_info['client_email']}` (編輯者)。")
        elif "File not found" in error_msg or "404" in error_msg:
            st.error(f"❌ 找不到資料夾：請確認 Secrets 中的 ID `{folder_id}` 是否正確。")
        else:
...
with col1: user_name = st.text_input("你的名字 (必填)", key="u_name")
with col2:
...
with col4: sugar = st.selectbox("甜度", SUGAR_OPTS, key="u_sugar")
with col5: ice = st.selectbox("冰塊", ICE_OPTS, key="u_ice")
...
if store_toppings:
    ...
    for pt in picked_tops:
        tn = pt.split(" (+")[0]
        topping_cost += store_toppings[tn]
...
if st.button("送出訂單", type="primary", use_container_width=True):
    if not user_name: st.error("❌ 請輸入名字！")
    else:
        try:
            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
if admin_mode:
    ...
    if len(raw_data) > 1:
        ...
        if not valid_idx:
            ...
            all_items = sorted(list({k for m in current_menus.values() for k in m.keys()}))
                        ...
                        try:
                            ...
                            if r_tops and r_store in all_toppings:
                                for t in r_tops.split(","):
                                    if t.strip() in all_toppings[r_store]: top_c += all_toppings[r_store][t.strip()]
                            new_p = base + top_c
            ...
            if balances is None:
                ...
                for _, row in spending.iterrows():
                    ...
                    report_data.append({
                        ...
                        "扣款後餘額": remain, "狀態": "✅ 足夠" if remain >= 0 else "❌ 不足"
                    })
                ...
                if report_data:
                    ...
                    edited_bal_df = st.data_editor(
                        ...
                        column_config={"扣款後餘額": st.column_config.NumberColumn("扣款後餘額", required=True, step=1)}
                    )
                    ...
                    if st.button("💸 確認扣款並更新儲值表 (End of Day)", type="primary"):
                        ...
                        try:
                            ...
                            else: st.error("儲值表欄位辨識失敗")
                        except Exception as e: st.error(f"結算失敗: {e}")
                else: st.info("今日無訂單需扣款")
